...
namespace wingman::silk::ingestion {
	...
	std::vector<std::string> ChunkPdfText(const std::string &pdfFilename, const int chunkSize, const int overlap)
	{
		...
		if (overlap >= chunkSize) {
			throw std::invalid_argument("Overlap cannot be greater than or equal to chunk size.");
		}

		const auto document = poppler::document::load_from_file(pdfFilename);
		if (!document) {
			// std::cerr << "Failed to open PDF: " << pdfFilename << std::endl;
			spdlog::error("Failed to open PDF: {}", pdfFilename);
		...
		for (int pageNum = 0; pageNum < document->pages(); ++pageNum) {
			...
			for (const auto &textbox : page->text_list()) {
				pdfContent += textbox.text().to_latin1();
				pdfContent += " ";
			}
		}

		pdfContent = FixUtf8String(pdfContent);

		// Chunk the entire PDF content with overlap
		for (size_t i = 0; i < pdfContent.size(); i += (chunkSize - overlap)) {
			const size_t end = std::min(i + chunkSize, pdfContent.size());
			chunks.push_back(pdfContent.substr(i, end - i));
		}
